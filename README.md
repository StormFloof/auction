...
Production-ready реализация многораундовой аукционной системы на Node.js + TypeScript, основанная на механике Telegram Gift Auctions.

> **📺 Для жюри конкурса:** см. [`DEMO.md`](DEMO.md) — демо-материалы, видео, работающий сайт
(https://youtu.be/Z4OW_gKjAgo) (https://auction-production-cfa9.up.railway.app/)

ТЕСТ K6 - https://youtu.be/qpi4NolTZCk
...
**Механика:**
- ✅ **Multi-round аукционы** — постепенный отсев участников
...
По умолчанию `npm run dev` запускает API **и встроенный scheduler** (inline worker), который автоматически закрывает раунды.
...
3) Для участников можно скопировать ссылку (кнопка **Скопировать ссылку**) — Dev Mode не нужен.

4) Ставка делается через кнопку **Сделать ставку** (bottom-sheet): можно вводить сумму вручную, использовать **+1 шаг / +2 шага / Максимум**, ошибки показываются понятным языком.
...
Аукцион поддерживает несколько лотов (призов). Участники соревнуются за top-N мест, где N = `lotsCount`.

### Ключевые поля
- `lotsCount` — количество призов/победителей (по умолчанию 1)
...
### Финализация
Аукцион завершается автоматически, когда `qualified.length <= lotsCount`:
- Все квалифицированные участники становятся победителями
- Hold конвертируется в capture (списание)
...
Anti-sniping: effective_end_at = max(oldEnd+extend, now+extend) при ставке внутри snipingWindow.
...
2) Поднять API (в одном терминале):
...
3) Поднять worker (в другом терминале):
...
4) Прогнать нагрузку (stdout + файл с summary):
...
Результаты: stdout; также сохраняется JSON-отчёт в [`contest-auction/load/summary.json`](contest-auction/load/summary.json:1).
...
В UI (User Mode) при создании аукциона есть переключатель **«Автоучастники: Вкл/Выкл»**.
В Dev Mode отображаются расширенные параметры (режим/количество/интервал) — они сохраняются в документе аукциона.
