...
Получить список выигрышей текущего участника (только завершённые аукционы).
...
4. Определяет победителей раунда (топ-`lotsCount`)
5. После завершения аукциона автоматически создает новый

**Механика закрытия раунда:**
- Топ-`lotsCount` участников получают приз (capture)
- Победители раунда выбывают из аукциона
- Все остальные продолжают с carry-over ставок
- Новые участники могут войти в следующий раунд
- Аукцион завершается когда `lotsDistributed >= totalLots`
...
### Anti-sniping
- Продление раунда ТОЛЬКО при конкурентной ставке
- Конкурентная ставка = обгоняет текущего лидера
- Защита от спама микроставками

### Модель распределения лотов
- `totalLots = lotsCount × maxRounds`
- Аукцион завершается когда все `totalLots` розданы
- Параметр `maxRounds` добавлен в форму создания
